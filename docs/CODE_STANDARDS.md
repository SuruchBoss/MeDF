...
- ถ้า `components/` ดึงของจาก `app/` มันจะผูกกับ route นั้นและใช้ซ้ำไม่ได้ —
  ซึ่งจะพังทันที เพราะทั้งแอปเป็น[เว็บสถิต](DEPLOYMENT.md) ที่ไม่มีเซิร์ฟเวอร์ให้พึ่ง
...
### เว็บสถิต: ข้อจำกัดที่ต้องจำ

ทั้งแอปเป็น `output: 'export'` จึง **ไม่มี server component ที่อ่านคุกกี้หรือ header ได้**

- ต้องการภาษาของผู้อ่าน → `useT()` ในคอมโพเนนต์ฝั่ง client เท่านั้น
- metadata ที่ต้องรู้ภาษาตั้งแต่ build → อ่าน `th` จากพจนานุกรมตรง ๆ
- **ห้ามอ่านค่าที่ต่างกันระหว่างเซิร์ฟเวอร์กับเบราว์เซอร์ระหว่าง render** —
  ถ้าจำเป็นให้ใช้ `useSyncExternalStore` แล้วให้ snapshot ฝั่งเซิร์ฟเวอร์ตรงกับ HTML
  (เคยเป็นบั๊กจริง: ตรวจภาษาจากเบราว์เซอร์ตอน render ทำให้คนอ่านภาษาอังกฤษเจอ React #418
  ทุกหน้า แล้ว React ทิ้ง HTML ที่ prerender ไว้ทั้งหน้า)
...
core เป็น **MIT** ทั้งหมด สิ่งเดียวที่ไม่อยู่ใน repo นี้คือโมดูลแก้ข้อความเดิมในไฟล์
ซึ่งต่อเข้ามาเป็น dependency ธรรมดา — ไม่มี guard ไม่มี hook ไม่มี scaffold

- ฟีเจอร์ทุกตัวประกาศใน `src/lib/features.ts` พร้อมแพ็กเกจต่ำสุดที่เข้าถึงได้
  **ประกาศเฉพาะสิ่งที่มีโค้ดอยู่จริง** — รายการที่ยังไม่ได้เขียนคือคำสัญญาที่ UI เอาไปโชว์
- การตรวจสิทธิ์ต้อง **fail closed** เสมอ — ไม่รู้จัก = ไม่อนุญาต และต้องใช้
  `Object.hasOwn` ไม่ใช่ `in` (ดูข้อ 9)
- อย่าคิดว่าโค้ดในเบราว์เซอร์ซ่อนได้ ลายน้ำของแพ็กเกจฟรีข้ามได้ด้วยการแก้ JavaScript
  และนั่นเป็นการแลกที่ตั้งใจ
...
### อะไรที่ทำตัวเป็น modal ต้องครบ 4 อย่าง

1. Escape ปิด
2. Tab วนอยู่ข้างใน ออกไปข้างหลังไม่ได้
3. ปิดแล้วโฟกัสกลับไปที่ปุ่มที่เปิดมัน
4. ตอนปิดต้อง `inert` — ไม่งั้นปุ่มที่เลื่อนออกนอกจอยังอยู่ใน tab order และ screen reader ยังอ่าน

**ใช้ `useDialog()` ได้ทั้ง 4 ข้อฟรี** เพราะมันเป็น `<dialog>` จริง
ที่ต้องทำเองมีกรณีเดียว: แผงคุณสมบัติของเอดิเตอร์ ซึ่งเป็นคอลัมน์ธรรมดาตั้งแต่ `lg` ขึ้นไป
และเป็นลิ้นชักต่ำกว่านั้น — `showModal()` จะโยนมันขึ้น top layer ทำแบบนั้นไม่ได้
กรณีแบบนี้ใช้ `useFocusTrap()` จาก `@/lib/client/use-focus-trap`

breakpoint ที่เปลี่ยน *พฤติกรรม* ไม่ใช่แค่สไตล์ ต้องรู้ค่าใน JS ด้วย (`inert` เป็น attribute
CSS เขียนไม่ได้) → `useMediaQuery()` จาก `@/lib/client/use-media-query`

✅ **บังคับด้วย `npm run test:ui` แล้ว** (ส่วน [19] ขับลิ้นชักจริงที่ 390px)

### ไอคอนมาจากที่เดียว

`node desktop/scripts/make-icon.mjs` วาดโลโก้จากเรขาคณิต 32 หน่วยชุดเดียว แล้วเขียนออก 6 ไฟล์:
ไอคอนแอป Windows (PNG + ICO), `favicon.ico`, `apple-icon.png`, `icon-192/512.png`
และ `icon-maskable.png` — **ห้ามวาดใหม่หรือหาไฟล์จากที่อื่นมาใส่**

ไฟล์ฝั่งเว็บถูก track ใน git (ต่างจากฝั่ง desktop ที่ ignore ไว้) เพราะ `next build`
ต้องไม่ขึ้นกับว่ามีใคร build desktop มาก่อนหรือยัง — **แก้โลโก้แล้วต้องรันสคริปต์นี้**

### ตัวเลขที่ต้องผ่าน

| เรื่อง | เกณฑ์ | ทำไม |
| --- | --- | --- |
| คอนทราสต์ข้อความ | **4.5:1** (ตัวใหญ่ ≥24px หรือ ≥18.66px ตัวหนา: 3:1) | WCAG 2.2 AA |
| คอนทราสต์ไอคอน/เส้นขอบ | 3:1 | ไอคอนไม่ใช่ข้อความ เกณฑ์ต่างกัน |
| ขนาดปุ่มบนจอสัมผัส | **≥24×24px** (แนะนำ 44×44) | WCAG 2.2 AA · นิ้วไม่ใช่เมาส์ |
| `<h1>` | ทุกหน้ามี **หนึ่งตัว** และห้ามข้ามระดับ | screen reader ใช้กระโดด |
| หน้าจอเลื่อนแนวนอน | **ห้ามมี** | บั๊กมือถือที่พบบ่อยที่สุด |

`text-ink-400` ใช้กับ **ไอคอนเท่านั้น** (3.45:1 — ผ่าน 3:1 ไม่ผ่าน 4.5:1)
ข้อความสีจาง ๆ ใช้ `text-ink-500` ขึ้นไป

✅ **บังคับด้วย `npm run test:ux` แล้ว** (อยู่ใน `npm test`)

### ขนาดหน้าจอที่ต้องรองรับ

| ชื่อ | ขนาด | คืออะไร |
| --- | --- | --- |
| `desktop` | 1440×900 | เว็บบนโน้ตบุ๊ก |
| `windows` | 1440×920 | หน้าต่างเริ่มต้นของแอป Windows |
| `windows-min` | **1024×680** | ขนาดเล็กที่สุดที่ย่อแอป Windows ได้ (`minWidth`/`minHeight`) |
| `tablet` | 768×1024 | |
| `mobile` | 390×844 | |
| `mobile-small` | 360×640 | Android รุ่นเล็ก |

`windows-min` คือขนาดที่ไม่มีใครลองด้วยมือ — ถ้าเลย์เอาต์พังตรงนี้ ผู้ใช้ Windows เจอแน่

**เครื่องมือแก้ไขต้องใช้ได้จริงบนมือถือ** แผงคุณสมบัติกว้าง 288px กินพื้นที่ 74% ของจอ 390px
จนเหลือที่ให้เอกสาร 102px — เคยเป็นแบบนั้นจริง ตอนนี้ต่ำกว่า `lg` มันเป็นลิ้นชัก (`fixed`)
ที่เปิดจากปุ่มบนแถบเครื่องมือ

---

## 10.1 ตรวจ UX/UI อัตโนมัติ
...
`apps/web/scripts/ux-audit.mjs` เปิดทุกหน้าในเบราว์เซอร์จริงแล้ววัดของจริง
ภาพหน้าจออยู่ที่ `apps/web/.ux-audit/` — **ดูภาพด้วย** เครื่องตรวจไม่เห็นว่าอะไร "ดูแปลก"

สิ่งที่มันจับได้ และเคยจับได้จริง:

- **ชื่อ key โผล่บนจอ** — `t()` ถูกใส่ที่ tooltip แต่ลืมที่ตัว label
  ปุ่มเครื่องมือ 9 ปุ่มจึงเขียนว่า `element.text` และรายการฟีเจอร์ในหน้าสมัครสมาชิก
  เขียนว่า `feature.exportPdf.label` — ทั้งสองจุดผ่าน type check และผ่านเทสต์ทุกตัว
  (เทียบกับรายการ key จริงใน `th.ts` จึงไม่มี false positive)
- คอนทราสต์ต่ำกว่าเกณฑ์ · ปุ่มเล็กเกินนิ้ว · หน้าเลื่อนแนวนอน · `id` ซ้ำ ·
  ข้อความถูกกล่องตัด · ปุ่มไม่มีชื่อให้ screen reader · ลำดับหัวข้อข้าม · error ในคอนโซล

**สีต้องอ่านผ่าน canvas ไม่ใช่ regex** Tailwind v4 เขียนสีเป็น `lab()` / `oklch()`
regex ที่จับแต่ `rgb()` จะคืน null แล้ว *ข้าม* องค์ประกอบนั้นเงียบ ๆ —
เครื่องตรวจที่ข้ามสิ่งที่ตัวเองมีหน้าที่ตรวจ แย่กว่าไม่มีเครื่องตรวจ
...
1. ประกาศใน `src/lib/features.ts` ด้วย `plan: 'paid'` — **หลังจาก** มีโค้ดแล้วเท่านั้น
2. ถ้าโค้ดอยู่ใน repo ส่วนตัว ให้ต่อเป็น dependency ปกติ
3. เพิ่มเทสต์ใน `tests/plans.test.ts` ว่าแพ็กเกจฟรีเข้าไม่ถึง — เส้นแบ่งนี้คือราคาทั้งหมดของสินค้า
</details>
