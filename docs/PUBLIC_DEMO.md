...
## เปิดใช้ครั้งแรก (ต้องทำครั้งเดียว)

**1. เปิด Pages ด้วยมือหนึ่งครั้ง**

ไปที่ **Settings → Pages → Build and deployment → Source** แล้วเลือก **GitHub Actions**

ขั้นนี้เลี่ยงไม่ได้: workflow ใส่ `enablement: true` ไว้แล้ว แต่ `GITHUB_TOKEN` ของ Actions
ไม่มีสิทธิ์สร้าง Pages site เอง (ตอบกลับว่า `Resource not accessible by integration`)
ต้องเป็นผู้ที่มีสิทธิ์ admin ของ repository กดเปิด

**2. รัน workflow**

- **Actions → Deploy demo to GitHub Pages → Run workflow** หรือ
- push อะไรก็ได้เข้า `main` ที่แตะ `apps/**`

หลังจากนี้ทุกครั้งที่ push เข้า `main` จะ deploy ให้เองโดยไม่ต้องทำอะไรอีก
